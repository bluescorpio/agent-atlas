...
export function buildAgentCard(
  ...
  return {
    name,
    description:
      `ERC-8183 Venus health-factor seller (${name}): given a borrower address, ` +
      `protocol (default Venus) and optional HF threshold, returns a spoken ` +
      `risk conclusion plus structured JSON from Venus APIs / Comptroller reads. ` +
      `Does not repay or add collateral.`,
